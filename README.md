...
> **Note for macOS Sequoia and later:** The previously recommended 'Webcam Settings' app no longer works. Instead, use **CameraController**, which is compatible with modern macOS versions and can control UVC camera registers that OpenCV cannot (such as exposure, white balance, contrast, etc.).
...
#### Launch CameraController

```bash
open /Applications/CameraController.app
```

- CameraController allows you to adjust advanced camera parameters for UVC cameras, including exposure, white balance, contrast, and more.
- The settings you apply in CameraController will persist while the camera is in use by this software.
- This is often more reliable than trying to control these parameters through OpenCV, especially on macOS where camera driver interfaces have more restrictions.

You can use CameraController to adjust your camera before starting recording with the ELP Camera control commands.
